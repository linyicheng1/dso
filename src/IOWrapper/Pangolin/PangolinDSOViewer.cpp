...
namespace dso
{
	namespace IOWrap
	{
		...
		void PangolinDSOViewer::drawTrace()
		{
			glLineWidth(1);
			glBegin(GL_LINES);
			...
			glColor3f(0.5, 0.5, 0);
			glBegin(GL_LINES);
			for (unsigned int i = 1; i < true_trace.size(); i++)
			{
				glVertex3f((GLfloat)true_trace.at(i-1)[0], (GLfloat)true_trace.at(i-1)[1], (GLfloat)true_trace.at(i-1)[2]);
				glVertex3f((GLfloat)true_trace.at(i)[0], (GLfloat)true_trace.at(i)[1], (GLfloat)true_trace.at(i)[2]);
			}
			glEnd();
		}
